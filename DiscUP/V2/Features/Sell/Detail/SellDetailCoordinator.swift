...
class SellDetailCoordinator: Coordinator<SellDetailCoordinator.Action> {
    ...
    enum Action {
        case editPhotosTapped(Binding<MarketItemV2>)
        case dismiss(save: Bool)
        case deleteItem
        case update(editableValue: MarketItemV2EditableValue, value: Any, shouldDismiss: Bool)
    }
    
    // MARK: - UIActions
    
    enum UIAction {
        case presentEditPhotos(Binding<MarketItemV2>)
        case dismiss
    }
...
extension SellDetailCoordinator {
    ...
    private func updateAction(
        editableValue: MarketItemV2EditableValue,
        value: Any,
        shouldDismiss: Bool
    ) {
        // BenDo: update DB
        ...
        DispatchQueue.main.async {
            ...
            case .itemType:
                guard let value = value as? MarketItemV2Type else { return }
